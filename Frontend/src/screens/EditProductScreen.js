...
const EditProductScreen = ({ route, navigation }) => {
  const { product, farm } = route.params;
  ...
  const [predictedYield, setPredictedYield] = useState(product.predicted_yield?.toString() || '');
  ...
  const [isNew, setIsNew] = useState(product.is_new !== undefined ? product.is_new : true);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      } else if (product.type === 'machinery') {
        if (producer.trim()) {
      ...
      switch (product.type) {
        case 'crop':
          response = await businessAPI.updateCrop(product.id, productData);
          break;
        case 'item':
          response = await businessAPI.updateItem(product.id, productData);
          break;
        case 'machinery':
          response = await businessAPI.updateMachinery(product.id, productData);
          break;
        default:
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.title}>Редактировать продукт</Text>
        <Text style={styles.subtitle}>Ферма: {farm.name}</Text>
        <Text style={styles.productType}>Тип: {getTypeLabel(product.type)}</Text>
      </View>
